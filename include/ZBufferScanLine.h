...
{
public:
    ZBufferScanLine(int width, int height, GLfloat near, GLfloat far);
    ...
    void  insertActiveEdgePairs(int lineIndex, ActivePolygon &polygon);
    ...
    void  insertPolygon(Geometry::Face *face, GeometryResource *geometry, bool useTexture);
...
private:
    // Preparation
    ZEdge         * generateEdge(glm::vec3 *p1, glm::vec3 *p2, int &top, int &bottom, glm::vec2 *tex1 = NULL, glm::vec2 *tex2 = NULL);

    ActiveEdgePair* generateEdgePair(ZEdge *left, ZEdge *right, ZPolygon &polygon);
